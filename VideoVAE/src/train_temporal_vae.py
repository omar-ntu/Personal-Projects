...
def train(args: argparse.Namespace) -> Dict[str, float]:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pbar = tqdm(train_loader, desc=f"TemporalVAE epoch {epoch}/{args.epochs}", leave=False)
        for batch in pbar:
    ...
    config["model_type"] = "temporal_vae"
...
@torch.no_grad()
def validate(model: TemporalVAE, loader, device: torch.device, beta: float) -> Dict[str, float]:
    model.eval()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out", type=str, default="checkpoints/temporal_vae.pt")
    parser.add_argument("--metrics-out", type=str, default="outputs/temporal_vae_metrics.json")
